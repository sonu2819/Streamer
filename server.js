...
// --- Explicit CORS for the video endpoint (for Chrome/Waveparty) ---
app.options('/movie.mp4', (req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, HEAD, OPTIONS, RANGE');
  res.setHeader('Access-Control-Allow-Headers', 'Range, Content-Range, Content-Type, Accept, Origin, User-Agent');
  res.setHeader('Access-Control-Expose-Headers', 'Content-Length, Content-Range, Accept-Ranges, Content-Type');
  res.setHeader('Access-Control-Allow-Private-Network', 'true'); // Chrome fix
...
async function getDriveClient() {
  ...
  // 1. Check for Environment Variable (Render/Railway)
  if (process.env.GOOGLE_SERVICE_ACCOUNT_KEY) {
    ...
    // 2. Fallback: Use the local JSON file (Local development)
    try {
      auth = new google.auth.GoogleAuth({
        keyFile: SERVICE_ACCOUNT_FILE,
        scopes: ["https://www.googleapis.com/auth/drive.readonly"],
